/**
 * Temple Rodgers - 2/05/2024
 * searchForFilesBool.gs
...
function driveLister(searchString) {
    ...
    // parse the search string to use AND between the words
    var words = searchString.split(' ');


    // Map each word to the required format and join them with 'AND'
    var queryString = words.map(function(word) {
        return ` AND fullText contains '${word}'`;
    }).join('');
    ...
    for (var i = 0; i < driveData.length; ++i) {
        generateDriveFiles(driveData[i],sheet,queryString);
    }
    //
 ...
 *
 */
function generateDriveFiles(drive,sheet,queryString) {
     ...
     *
     * */
    const filesQuery = "trashed = false AND mimeType != \'application/vnd.google-apps.folder\'" + queryString;
    // use this version of filesQuery for search with regex
    // const filesQuery = "trashed = false AND fullText contains " + queryString + " AND mimeType != 'application/vnd.google-apps.folder'";
    // call the search
    Logger.log(filesQuery);
